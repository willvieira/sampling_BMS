...
To select the hexagons for the second year, we have to adjust the sample size ($\sigma_{\text{adj N}}$) and the inclusion probability ($\sigma_{\text{adj p}}$) in function of a defined buffer size ($\sigma$).
...
for(eco in eco_sim)
{
    ...
    p2 <- ggplot() +
        ...
        ggtitle(paste0("**<span style='color:blue;'>Selected</span>** (", nrow(subset(hexas, ecoregion == eco  & ET_Index %in% out_sample_10000$sites_base$ET_Index)), ") hexagons year 2 (*σ<sub>adj N</sub>* = 10; *σ<sub>adj p</sub>* = 10)")) +
        theme(plot.title = element_markdown())
    ...
    p3 <- ggplot() +
        ...
        ggtitle(paste0("**<span style='color:blue;'>Selected</span>** (", nrow(subset(hexas, ecoregion == eco  & ET_Index %in% out_sample$sites_base$ET_Index)), ") hexagons year 2 (*σ<sub>adj N</sub>* = ", round(mean(best_bufferSize$best_bs), 1), "; *σ<sub>adj p</sub>* = ", round(mean(best_bufferSize$best_bs), 1), ")")) +
        theme(plot.title = element_markdown())
    ...
    p4 <- ggplot() +
        ...
        ggtitle(paste0("**<span style='color:blue;'>Selected</span>** (", nrow(subset(hexas, ecoregion == eco  & ET_Index %in% out_sample_adj10000$sites_base$ET_Index)), ") hexagons year 2 (*σ<sub>adj N</sub>* = ", round(mean(best_bufferSize$best_bs), 1), "; *σ<sub>adj p</sub>* = 10)")) +
        theme(plot.title = element_markdown())
    ...
    for(Ref in c('S', 'NS')) {
        dist_dt <- rbind(
            dist_dt,
            data.frame(
                sim = "<span style='color:blue;'>Selected</span><br>(*σ<sub>adj N</sub>*:10; *σ<sub>adj p</sub>*: 10)",
                ref = Ref,
                dist = apply(
                    st_distance(
    ...
    for(Ref in c('S', 'NS')) {
        dist_dt <- rbind(
            dist_dt, 
            data.frame(
                sim = paste0("<span style='color:blue;'>Selected</span><br>(*σ<sub>adj N</sub>*: ", round(mean(best_bufferSize$best_bs), 1), "; *σ<sub>adj p</sub>*: ", round(mean(best_bufferSize$best_bs), 1), ")"),
                ref = Ref,
                dist = apply(
                    st_distance(
                        get(paste0('legacy', Ref)),
    ...
    for(Ref in c('S', 'NS')) {
        dist_dt <- rbind(
                dist_dt, 
                data.frame(
                    sim = paste0("<span style='color:blue;'>Selected</span><br>(*σ<sub>adj N</sub>*: ", round(mean(best_bufferSize$best_bs), 1), "; *σ<sub>adj p</sub>*: 10)"),
                    ref = Ref,
                    dist = apply(
                        st_distance(
                            get(paste0('legacy', Ref)),
