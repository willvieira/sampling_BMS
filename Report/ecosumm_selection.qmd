...
The GRTS method selects one main hexagon (marked in red) and one over hexagon (blue) for each ecoregion.
As the over hexagon is chosen randomly, we additionally select an extra hexagon (yellow) next to each main hexagon with the highest inclusion probability among its neighboring hexagons.
