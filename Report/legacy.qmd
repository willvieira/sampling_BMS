...
#| fig-cap: "Legacy effect in the inclusion probabiity of neighbouring sites in function their Euclian distance. Each line represents the effect range of legacy sites. Figure adapted from the vignette 'An Introduction to MBHdesign' found in @Foster2021."
...
Here we propose a novel approach where, in addition to modifying the inclusion probability, we also adjust the sample size according to the distribution of historical sites.
...
```{r adj_sampleSize2,echo=FALSE,message=FALSE,warning=FALSE,fig.width=8,fig.height=4}
#| label: fig-adj-sampleSize2
#| fig-cap: "Grid of sample units and historical sites spatially balanced (blue) and unbalanced (red) for a simulated region. Using the same approach for adjusting the inclusion probability, a buffer around each historical site is created to calculate the adjusted sampled size."
...
p1 <- ggplot(fake_hex) +
    geom_sf(fill = 'white', color = 'grey') +
    geom_sf(data = fake_hex[balanced_ls], fill = '#1A85FF', color = 'grey') +
    geom_sf(data = st_union(st_buffer(fake_hex[balanced_ls], dist = buffSize)), fill = '#1A85FF', color = 'transparent', alpha = .4) +
    theme_minimal() +
    xlim(.25, 7.8) + ylim(.25, 7.75) +
    theme(
        axis.text.x=element_blank(),
...
p2 <- ggplot(fake_hex) +
    geom_sf(fill = 'white', color = 'grey') +
    geom_sf(data = fake_hex[unbalanced_ls], fill = '#D41159', color = 'grey') +
    geom_sf(data = st_union(st_buffer(fake_hex[unbalanced_ls], dist = buffSize)), fill = '#D41159', color = 'transparent', alpha = .4) +
    theme_minimal() +
    xlim(.25, 7.8) + ylim(.25, 7.75) +
    theme(
        axis.text.x=element_blank(),
...
nbHexas_buffer |>
    ...
    ggplot(aes(buffer_size, N_adj)) +
        ...
        geom_text(aes(x = 20.5, y = nrow(hexas_sim) * sample_effort -20 + 5, label = as.character(as.expression(eq1))), parse = TRUE, hjust = 0, check_overlap = TRUE) +
        geom_text(aes(x = 20.5, y = nrow(hexas_sim) * sample_effort + 5, label = as.character(as.expression(eq2))), parse = TRUE, hjust = 0, check_overlap = TRUE) +
        geom_text(aes(x = opt_bufferSize - 1.5, y = 50.5, label = 'Optimum buffer size'), hjust = 0, angle = 90, check_overlap = TRUE) +
        theme_minimal() +
        theme(legend.position = 'none') +
        xlab('Buffer size (Km)') +
...
```{r figBufferVsNadjForEcoregion,echo=FALSE,message=FALSE,warning=FALSE,fig.width=8,fig.height=7}
...
```{r figPropvsNadj_ecoregion,echo=FALSE,message=FALSE,warning=FALSE,fig.width=8,fig.height=7}
...
```{r figOptBufferSizevsEco,echo=FALSE,message=FALSE,warning=FALSE,fig.width=6,fig.height=7}
...
```{r figEcosExample,echo=FALSE,message=FALSE,warning=FALSE,fig.width=7,fig.height=7}
#| label: fig-ecosExample
#| fig-cap: "Distribution of historical sites and their coverage according to the optimal buffer size specific for the ecoregion."
...
hexas |>
    ...
    ggplot() +
        ...
        geom_sf(
            data = hexas |>
                    filter(ET_Index %in% subset(nbLegacy_hex, nbLegacy > 0)$ET_Index),
            fill = 'red',
            color = NA
        ) +
        geom_textbox(
            aes(x = -73.5, y = 52.2, label = "**Ecoregion 217**<br><span style='color:red'>Historical sites</span>: 96<br>Initial sample size: 56<br>Adjuted sample size: 22"),
            width = .30
        ) +
        geom_textbox(
            aes(x = -78.5, y = 49, label = "**Ecoregion 99**<br><span style='color:red'>Historical sites</span>: 579<br>Initial sample size: 72<br>Adjuted sample size: 3"),
            width = .30
        ) +
        theme_minimal() +
        coord_sf(crs = 4326) +
...
In order to achieve an optimal design, additional simulations may be required to fine-tune the parameters.
While we have accounted for spatial coverage in adjusting the sample size, this approach could be also extended to include habitat distribution in order to better account the effect of legacy sites in the sampling design.
