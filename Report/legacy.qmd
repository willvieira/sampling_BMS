...
Every PSU hexagon is assigend an inclusion probability as a function of habitat and access cost.
...
```{r fosterGaussian,echo=FALSE,message=FALSE,warning=FALSE,fig.width=8,fig.height=6}
...
The method to reduce the inclusion probability of neighbouring sites near legacy sites developed in @Foster2021 is implimented in the R package `MBHdesign`.
...
```{r legacyExample,echo=FALSE,message=FALSE,warning=FALSE,fig.width=10,fig.height=8}
...
incl_ls <- readRDS(file.path('data', 'inclPorb_figFoster.RDS'))
...
We applied the same approach for two contrasting ecoregion in terms of spatial distribution of historical sites.
By using the spatially balanced design while ignoring historical sites, the selected hexagons are evenly distributed across the ecoregion, with overlapping the historical sites (@fig-sampleSize-example1).
Adjusting the inclusion probability of hexagons given their distance to historical sites increases the spatial coverage of underrepresented areas (@fig-sampleSize-example2).
...
The issue is that when an ecoregion is well covered by historical sites, the sampled hexagons are forced to concentrate in smaller available areas.
For ecoregion 99, almost a third of the sampled hexagons are concentrated in a 20 km buffer when the inclusion probability was adjusted.
