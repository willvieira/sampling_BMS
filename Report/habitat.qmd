# Habitat information {#sec-habitat-info}
...
```{r sampleHabitat,echo=FALSE,message=FALSE,warning=FALSE,fig.width=8,fig.height=7}
#| label: fig-sample-habitat
#| fig-cap: "Example of sampling 10 (2%) hexagons in ecoregion 102 with (left bar) and without (right) weighted probability accounting for habitat heterogenity. Eeach bar represents the relative proportion of habitat classes from the 10 selected hexagons."
    ...
    mutate(
        sampled = factor(
            ifelse(
                ...
                'Random sample',
                ifelse(ET_Index %in% sample_prob, 'Weighted sample', 'WRONG!')
            )
        )
    ) |>
