...
for(id in unique(districts$ECOREGION))
{
    ...
    # plot
    par(mfrow = c(1, 2), mar = c(2.5, 2.5, 2.5, 0.5), mgp = c(1.4, 0.2, 0), tck = -.008)
    hist(hexa_ecoregion$hab_ca, main = '', ylab = 'Frequency (# of hexagons)', xlab = 'Habitat probability of hexagons', breaks = 30)
    hist(hexa_ecoregion$propNA * 100, main = '', ylab = 'Frequency (# of hexagons)', xlab = 'Proportion of NA pixels within each hexagon (%)', breaks = 30)
